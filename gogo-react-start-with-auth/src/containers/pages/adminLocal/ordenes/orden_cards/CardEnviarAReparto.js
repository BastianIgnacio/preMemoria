...
import { ORDEN_ENVIAR_A_REPARTO } from '../../../../../redux/actions';

const CardEnviarAReparto = ({ modal, orden }) => {
  ...
  const [modalDetail, setModalDetail] = useState(
    'La orden está por ser despachada a Delivery'
  );
  const [modalTittle, setModalTittle] = useState('Despachando a Delivery!');
  const [modalIcon, setModalIcon] = useState('iconsminds-clock-forward');

  const ordenEnviada = () => {
    setActualizando(false);
    setModalDetail('La orden ya se encuentra en proceso de delivery!');
    setModalTittle('DESPACHADO A DELIVERY');
    setModalIcon('iconsminds-scooter');
    setPercent(100);
    setProgressText('100%');
  ...
  const cancelar = () => {
    setActualizando(false);
    setModalDetail('El despacho a delivery ha sido cancelado!');
...
export default React.memo(CardEnviarAReparto);
